...
function SuperAdminContent() {
  ...
  // Hero Image state
  const [newHero, setNewHero] = useState<string | null>(null); // MODIFICATION: This is now JUST for preview
  const [finalHeroUrl, setFinalHeroUrl] = useState<string | null>(null); // MODIFICATION: This is for saving
  ...
  // MODIFICATION: Add a reusable handler for canceling/closing the form
  const handleCancel = () => {
    ...
    setNewDesc("");
    setNewHero(null);
    setFinalHeroUrl(null); // <-- Clear final URL
  };
  ...
  // NOTE: I have removed the duplicate "Create New Template" block 
  // that was outside the `showAddTemplate` conditional.
            ...
            {activeTab === 'templates' && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button
                    onClick={() => { setShowAddTemplate(true); setEditing(null); setNewHero(null); setFinalHeroUrl(null); }}
                ...
                {showAddTemplate && (
                  <div className="mb-6 bg-gray-50 rounded-xl p-6 border border-gray-200">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button
                        onClick={handleCancel} // <-- MODIFICATION: Use handler
                      ...
                      <div className="grid sm:grid-cols-[160px_1fr] gap-4 items-start">
                        ...
                        <FileUpload
                          ...
                          onUploadComplete={(url: string, fileName: string) => {
                            setNewHero(url); // <-- Update preview to permanent URL
                            setFinalHeroUrl(url); // <-- SET THE FINAL URL FOR SAVING
                    ...
                    {/* NOTE: I have removed the duplicate/broken button that was here */}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        {/* MODIFICATION: This is the updated Create/Save button */}
                        <button
                          onClick={async () => {
                            ...
                            try {
                              const templateData = {
                                heroImage: finalHeroUrl, // <-- Save the FINAL URL
                              ...
                              handleCancel(); // <-- Use handler to reset
                          ...
                          // Disable if busy or if no FINAL URL
                          disabled={tplBusy || !finalHeroUrl}
                        ...
                        return (
                          <div key={template.id} className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                            ...
                            <div className="flex gap-2">
                              {/* MODIFICATION: Added onClick handler for Edit */}
                              <button 
                                onClick={() => {
                                  // Populate the form with the template's data
                                  setNewTitle(template.title || "");
                                  setNewDesc(template.description || "");
                                  setNewHero(template.heroImage || null);
                                  setFinalHeroUrl(template.heroImage || null); // <-- Also set the final URL
                                  
                                  // Set the *editing* state to this template object
                                  setEditing(template);
